...
function toggleOtherMemberField() {
  const sel = document.getElementById('memberName');
  const wrap = document.getElementById('otherMemberNameWrap');
  if (!sel || !wrap) return;
  wrap.style.display = sel.value === 'Other' ? 'block' : 'none';
}
...
function addCommitteeApproval() {
  ...
  let member = memberSel ? memberSel.value : '';
  if (member === 'Other') {
    const otherMemberInput = document.getElementById('otherMemberName');
    member = (otherMemberInput?.value?.trim()) || '';
  }

  const member2 = document.getElementById('memberName2')?.value?.trim() || '';
  const otherMember2 = document.getElementById('otherMemberName2')?.value?.trim() || '';
  // Ensure otherMember is defined when "Other" is selected for member
  const otherMember = (memberSel && memberSel.value === 'Other')
    ? (document.getElementById('otherMemberName')?.value?.trim() || '')
    : '';
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .prepared-row { display: flex; align-items: center; justify-content: space-between; gap: 30px; margin-top: 35px; margin-bottom: 18px; font-size: 12px; }
          .prepared-field { flex: 1; white-space: nowrap; }
          .prepared-field.date-field { flex: 0 0 42%; }
          .prepared-field strong { margin-right: 6px; }
          .prepared-line { border-bottom: 1px dotted #000; display: inline-block; width: 260px; height: 14px; vertical-align: bottom; }
          .prepared-field.date-field .prepared-line { width: 220px; }
          .committee-heading-spacer { margin-top: 60px; }
      ...
      <body>
        ...
        <div class="prepared-row">
          <div class="prepared-field"><strong>Prepared by:</strong><span class="prepared-line"></span></div>
          <div class="prepared-field date-field"><strong>Date:</strong><span class="prepared-line"></span></div>
        </div>

        ${jobCardsHtml}

        <div class="committee-heading-spacer"></div>
        ...
        <table class="committee-table">
          <tbody>
            ...
            <tr>
              <th>MEMBER</th>
              <td class="colon">:</td>
              <td class="name">${e(approval.member2 || '')}</td>
              <td class="colon">:</td>
              <td class="signature"><span class="signature-line"></span></td>
            </tr>
            <tr>
              <th>MEMBER</th>
              <td class="colon">:</td>
              <td class="name">${e(approval.otherMember2 || '')}</td>
              <td class="colon">:</td>
              <td class="signature"><span class="signature-line"></span></td>
            </tr>
...
function showPage(pageId) {
  ...
  } else if (pageId === 'committeeTotalsPage') {
    initCommitteeTotalsPage();
    renderCommitteeTotals();
  }
}
...
// ===== Committee Approval Totals (CM Total) =====
function initCommitteeTotalsPage() {
  const yearInput = document.getElementById('cmTotalsYear');
  const now = new Date();
  if (yearInput && !yearInput.value) {
    yearInput.value = now.getFullYear();
  }

  // Wire up period radio controls to show/hide extra selectors
  updateCommitteeTotalsPeriodControls();

  const periodRadios = document.querySelectorAll('input[name="cmTotalsPeriod"]');
  if (periodRadios && periodRadios.length) {
    periodRadios.forEach(r => {
      r.removeEventListener('change', updateCommitteeTotalsPeriodControls);
      r.addEventListener('change', updateCommitteeTotalsPeriodControls);
    });
  }
}

function updateCommitteeTotalsPeriodControls() {
  const selected = document.querySelector('input[name="cmTotalsPeriod"]:checked');
  const period = selected ? selected.value : 'fullYear';

  const monthWrap = document.querySelector('.cm-period-month');
  const halfWrap = document.querySelector('.cm-period-half');

  if (monthWrap) monthWrap.classList.toggle('d-none', period !== 'month');
  if (halfWrap) halfWrap.classList.toggle('d-none', period !== 'half');
}

function getCommitteeTotalsRange() {
  const yearInput = document.getElementById('cmTotalsYear');
  const now = new Date();
  const year = parseInt(yearInput && yearInput.value ? yearInput.value : now.getFullYear(), 10) || now.getFullYear();

  const selected = document.querySelector('input[name="cmTotalsPeriod"]:checked');
  const period = selected ? selected.value : 'fullYear';

  const monthSelect = document.getElementById('cmTotalsMonth');
  const halfSelect = document.getElementById('cmTotalsHalf');

  let start, end, label;

  if (period === 'month') {
    const m = parseInt(monthSelect && monthSelect.value ? monthSelect.value : (now.getMonth() + 1), 10);
    start = new Date(year, m - 1, 1);
    end = new Date(year, m, 0);
    const monthText = monthSelect && monthSelect.options && monthSelect.selectedIndex >= 0
      ? monthSelect.options[monthSelect.selectedIndex].text
      : `Month ${m}`;
    label = `${monthText} ${year}`;
  } else if (period === 'half') {
    const hv = halfSelect && halfSelect.value ? halfSelect.value : 'H1';
    if (hv === 'H1') {
      start = new Date(year, 0, 1);
      end = new Date(year, 6, 0); // end of June
      label = `First Half of ${year}`;
    } else {
      start = new Date(year, 6, 1); // July 1
      end = new Date(year, 12, 0);  // Dec 31
      label = `Second Half of ${year}`;
    }
  } else if (period === 'firstThree') {
    start = new Date(year, 0, 1);
    end = new Date(year, 3, 0); // end of March
    label = `First 3 Months of ${year}`;
  } else {
    // full year
    start = new Date(year, 0, 1);
...
function renderCommitteeTotals() {
  const summaryDiv = document.getElementById('cmTotalsSummary');
  const tbody = document.getElementById('cmTotalsTableBody');
  const rangeLabelEl = document.getElementById('cmTotalsRangeLabel');

  if (!summaryDiv || !tbody) return;

  const { start, end, label } = getCommitteeTotalsRange();
  ...
  if (!Array.isArray(committeeApprovals) || committeeApprovals.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="6" class="text-center text-muted">No Committee Approvals found.</td>
      </tr>
    `;
    summaryDiv.innerHTML = '<p class="text-muted mb-0">No Committee Approvals are available in the system.</p>';
    return;
  }
  ...
  committeeApprovals.forEach(appr => {
    const dateStr = appr.date || '';
    if (!dateStr) return;
    ...
    const t = typeof appr.totalAmount !== 'undefined' ? appr.totalAmount : 0;
    const v = typeof appr.vatAmount !== 'undefined' ? appr.vatAmount : 0;
    const gt = typeof appr.totalAmountWithVat !== 'undefined'
      ? appr.totalAmountWithVat
      : (parseNumber(t) + parseNumber(v));
    ...
    const mr = appr.mrNumber || appr.mr || '';
    const pr = appr.prNumber || appr.pr || '';
    ...
    rowsHtml.push(`
      <tr>
        ...
        <td>${mr || '-'}</td>
        <td>${pr || '-'}</td>
        <td class="text-end">${formatMoney(tNum)}</td>
        <td class="text-end">${formatMoney(vNum)}</td>
        <td class="text-end">${formatMoney(gtNum)}</td>
      </tr>
    `);
  });
  ...
  if (count === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="6" class="text-center text-muted">No approvals found for the selected period.</td>
      </tr>
    `;
    summaryDiv.innerHTML = `
      <p class="mb-0 text-muted">No Committee Approvals matched the selected period.</p>
    `;
    return;
  }
  ...
  tbody.innerHTML = rowsHtml.join('') + `
    <tr class="fw-bold table-secondary">
      ...
      <td class="text-end">${formatMoney(vatTotal)}</td>
  ...
  summaryDiv.innerHTML = `
    <ul class="list-unstyled mb-0">
      <li><strong>Approvals Count:</strong> ${count}</li>
      <li><strong>Total (Excl. VAT):</strong> ${formatMoney(total)}</li>
      <li><strong>VAT Total:</strong> ${formatMoney(vatTotal)}</li>
      <li><strong>Total (Incl. VAT):</strong> ${formatMoney(grandTotal)}</li>
    </ul>
  `;
}
