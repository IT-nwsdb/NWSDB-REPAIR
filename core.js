...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Member select toggle for Other
  const memberSelect = document.getElementById('memberName');
  if (memberSelect) {
    memberSelect.addEventListener('change', toggleOtherMemberField);
    toggleOtherMemberField();
  }
  ...
  if (systemTrackingSearch) {
    ...
    // Committee Totals (CM Total) button
  const cmTotalsBtn = document.getElementById('cmTotalsCalculateBtn');
  if (cmTotalsBtn) {
    cmTotalsBtn.addEventListener('click', function (e) {
      e.preventDefault();
      renderCommitteeTotals();
    });
  }

  // Committee Totals period radios
  const cmTotalsRadios = document.querySelectorAll('input[name="cmTotalsPeriod"]');
  if (cmTotalsRadios && cmTotalsRadios.length) {
    cmTotalsRadios.forEach(r => {
